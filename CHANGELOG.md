...
* Added new `FeatureExtraction` type for storing features extracted from electrical recordings, including the `features`, `times`, and per-feature `description` datasets and an `electrodes` `DynamicTableRegion` referencing the `ElectrodesTable`. (@cboulay)
* **[BREAKING]** Added `BaseIO::appendReferenceDataSet` pure virtual method, implemented by `HDF5IO`, to append object references to an existing extensible reference dataset. External `BaseIO` implementations must implement the new method. (@cboulay)
* Added support for appending to an existing `ElectrodesTable` across repeated `addElectrodes()`/`finalize()` cycles, including from a new `ElectrodesTable` object created for a previously written file. String columns are appended at their current extent via the new `DynamicTable::columnWriteOffset` hook and the `group` reference column is extended in place (see [#313](https://github.com/NeurodataWithoutBorders/aqnwb/issues/313)). (@cboulay, @chittti)
...
* Relaxed `DynamicTable::setColNames` (previously restricted to reordering) to also allow appending new column names; removing existing names or passing duplicate names throws `std::invalid_argument`. (@chittti)
* Updated `HDF5IO::createReferenceDataSet` to create chunked, extensible datasets so that reference columns can be appended to. `DynamicTable::addReferenceColumn` now appends to an existing reference column instead of always creating a new one. (@cboulay)
